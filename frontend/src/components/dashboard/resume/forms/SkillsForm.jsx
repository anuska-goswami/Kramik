...
export function SkillsForm({ data, onChange }) {
  ...
  return <div className="space-y-8">
      {categories.map((cat) => <div key={cat.id} className="bg-[#0A0E17]/60 border border-white/10 rounded-2xl p-5">
          <label className="block text-xs font-semibold text-gray-400 mb-2 uppercase tracking-wider">{cat.label}</label>
          
          <div className="flex flex-wrap gap-2 mb-3">
            {data[cat.id].map((skill, idx) => <div key={idx} className="flex items-center gap-1.5 px-3 py-1 bg-[#B5FF45]/10 text-[#B5FF45] border border-[#B5FF45]/20 rounded-lg text-sm">
                <span>{skill}</span>
  ...
  >
          ...
          {cat.aiSuggestions.length > 0 && <div className="mt-3 flex items-center gap-2 flex-wrap">
              <Sparkles className="w-3.5 h-3.5 text-[#B5FF45]" />
              <span className="text-xs text-gray-500">Suggestions:</span>
              {cat.aiSuggestions.filter((s) => !data[cat.id].includes(s)).map((s, idx) => <button
    key={idx}
    onClick={() => {
