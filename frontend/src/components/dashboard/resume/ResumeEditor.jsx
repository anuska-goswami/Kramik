...
export function ResumeEditor({ onBack }) {
  ...
  return <div className="h-full flex flex-col max-w-[1600px] mx-auto text-white overflow-hidden">
      {
    /* Top Header */
  }
  ...
  >
    ...
    return <div key={step.id} className="flex items-center gap-2">
      ...
      className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${isActive ? "bg-[#B5FF45]/10 text-[#B5FF45] border border-[#B5FF45]/20" : isCompleted ? "text-gray-300 hover:bg-white/5" : "text-gray-600 hover:text-gray-400"}`}
    >
                      {isCompleted ? <Check className="w-3.5 h-3.5" /> : <step.icon className="w-3.5 h-3.5" />}
                      {step.title}
                    </button>
                    {idx < steps.length - 1 && <ChevronRight className="w-4 h-4 text-gray-700 shrink-0" />}
                  </div>;
  })}
            </div>
          </div>
  ...
  >
    ...
    className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm transition-all ${activeStepIndex === 0 ? "opacity-0 pointer-events-none" : "bg-white/5 text-white hover:bg-white/10"}`}
  >
                    <ChevronLeft className="w-4 h-4" /> Back
                  </button>
                  <button
    onClick={handleNext}
    className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm transition-all ${activeStepIndex === steps.length - 1 ? "bg-[#B5FF45]/10 text-[#B5FF45] border border-[#B5FF45]/20 hover:bg-[#B5FF45] hover:text-[#05080D]" : "bg-white text-[#05080D] hover:bg-gray-200"}`}
  >
