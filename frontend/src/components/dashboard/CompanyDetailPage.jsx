...
export function CompanyDetailPage({ onBack }) {
  const { id } = useParams();
  ...
  >
    ...
    /* Tabs */
      ...
      <div className="flex items-center gap-2 overflow-x-auto border-b border-border-custom pb-2 hide-scrollbar">
        {["Overview", "Recruitment Process", "Preparation Roadmap", "Interview Experiences", "Mock Tests", "Resources"].map((tab) => <button
    key={tab}
    onClick={() => setActiveTab(tab)}
    className={`px-5 py-3 rounded-xl text-sm font-semibold whitespace-nowrap transition-all duration-300 ${activeTab === tab ? "bg-[#B5FF45]/10 text-[#B5FF45]" : "text-gray-400 hover:text-white hover:bg-white/5"}`}
  >
            {tab}
          </button>)}
      </div>
  ...
  >
    ...
    { label: "Interview Readiness", value: 30 }
          ...
          {activeTab === "Recruitment Process" && <div className="space-y-6">
              <div className="bg-[#0A0E17]/60 border border-white/5 rounded-2xl p-6 lg:p-8">
                ...
                <div className="relative border-l-2 border-white/10 ml-4 lg:ml-0 lg:border-l-0 lg:border-t-2 lg:flex justify-between pt-8 lg:pt-12 pb-4 space-y-10 lg:space-y-0">
                  {company.hiringProcess.map((round, idx) => <div key={idx} className="relative lg:w-1/4 px-6 lg:px-4 text-left lg:text-center">
          ...
          {activeTab === "Preparation Roadmap" && <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
    ...
    { title: "HR Interview Questions", progress: 0, status: "Not Started", icon: MessageSquare }
  ].map((phase, idx) => <div key={idx} className="bg-[#0A0E17]/60 border border-white/5 rounded-2xl p-6 flex flex-col justify-between hover:border-white/20 transition-colors group">
                  <div>
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${phase.progress === 100 ? "bg-[#B5FF45]/10 text-[#B5FF45]" : phase.progress > 0 ? "bg-blue-500/10 text-blue-400" : "bg-white/5 text-gray-500"}`}>
                        {phase.status}
                      </span>
                    </div>
          ...
          {activeTab === "Interview Experiences" && <div className="space-y-10">
              <div className="space-y-4">
                <h3 className="text-xl font-bold text-white mb-6">Previous Year Experiences</h3>
                {[1, 2, 3].map((_, idx) => <div key={idx} className="bg-[#0A0E17]/60 border border-white/5 rounded-2xl overflow-hidden">
                    <div
  ...
  >
                    ...
                    <AnimatePresence>
                      {expandedRounds.includes(idx) && <motion.div
    initial={{ height: 0 }}
    animate={{ height: "auto" }}
    exit={{ height: 0 }}
  ...
  >
    ...
    { category: "Behavioral Questions", q: "Describe a situation where you had to meet a tight deadline." }
  ].map((faq, idx) => <div key={idx} className="bg-[#0A0E17]/60 border border-white/5 rounded-2xl p-6 hover:border-white/10 transition-colors">
                    <div className="flex justify-between items-start mb-4">
          ...
          {activeTab === "Mock Tests" && <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[
    ...
    { title: "Technical MCQ Round", questions: 40, duration: "60m", difficulty: "Medium" }
  ].map((test, idx) => <div key={idx} className="bg-[#0A0E17]/60 border border-white/5 rounded-2xl p-6 hover:border-[#B5FF45]/50 transition-colors group flex flex-col">
          ...
          {activeTab === "Resources" && <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {[
  ...
  ].map((resource, idx) => <div key={idx} className="bg-[#0A0E17]/60 border border-white/5 rounded-2xl p-5 flex items-center justify-between hover:bg-white/[0.02] transition-colors cursor-pointer group">
