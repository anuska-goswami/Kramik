...
import { 
  ArrowRight, Users, Terminal, MessageSquare, FileText, 
  Target, Mic, Briefcase, BookOpen, Clock, PlayCircle,
  TrendingUp, CheckCircle2, Sparkles, Building2, Layout, Map
} from 'lucide-react';
import { interviewCategories, mockInterviewsData, interviewExperiencesData, resourcesData } from '../../data/interviewData';

const iconMap: Record<string, React.ElementType> = {
  Users, Terminal, MessageSquare, FileText, Target, Mic, Briefcase, UsersRound: Users
};

const resourceIconMap: Record<string, React.ElementType> = {
  FileText, Terminal, PlayCircle, Users
};

export function InterviewPrepContent({ onCategorySelect }: { onCategorySelect?: () => void }) {
    ...
    >
      ...
      <motion.div variants={{ hidden: { opacity: 0 }, visible: { opacity: 1 } }} className="pt-8 border-t border-border-custom">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            ...
            <div key={idx} className="bg-gradient-to-br from-[#B5FF45]/10 to-transparent border border-[#B5FF45]/30 rounded-2xl p-5 flex flex-col justify-between hover:border-[#B5FF45]/50 transition-colors group">
      ...
      <motion.div variants={{ hidden: { opacity: 0 }, visible: { opacity: 1 } }} className="pt-8 border-t border-border-custom">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {mockInterviewsData.map((mock) => (
            <div key={mock.id} className="bg-[#0A0E17]/60 border border-white/5 rounded-2xl p-5 hover:border-white/20 transition-colors group flex flex-col">
      ...
      <motion.div variants={{ hidden: { opacity: 0 }, visible: { opacity: 1 } }} className="pt-8 border-t border-border-custom">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {interviewExperiencesData.map((exp) => (
            <div key={exp.id} className="bg-[#0A0E17]/60 border border-white/5 rounded-2xl p-6 hover:border-white/20 transition-colors flex flex-col">
              <div className="flex justify-between items-start mb-4">
                <div>
      ...
      <motion.div variants={{ hidden: { opacity: 0 }, visible: { opacity: 1 } }} className="pt-8 border-t border-border-custom">
        ...
        <div className="flex flex-wrap gap-3">
          {['TCS', 'Infosys', 'Wipro', 'Cognizant', 'Accenture', 'Capgemini', 'Deloitte', 'Amazon', 'Microsoft', 'Google', 'Adobe', 'Oracle'].map((company, idx) => (
            <button key={idx} className="px-5 py-3 bg-[#0A0E17]/60 border border-white/5 hover:border-[#B5FF45]/50 hover:bg-[#B5FF45]/5 rounded-xl text-sm font-semibold text-white transition-all group flex items-center gap-2">
