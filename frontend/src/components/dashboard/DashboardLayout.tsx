...
import {
  LayoutDashboard, Map, Calendar, FileEdit, BookOpen,
  BarChart3, Brain, Trophy, User, Settings, Bell, MessageSquare,
  Search, Menu, X, ChevronRight, Moon, Sun, Sparkles, Code, Briefcase, FileText, Bookmark
} from 'lucide-react';
...
export function DashboardLayout({ onSignOut, onNavigateToProfile, initialTab = 'dashboard' }: DashboardLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-bg-primary text-text-primary flex overflow-hidden font-sans selection:bg-[#B5FF45]/30">
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 rounded-xl transition-all duration-300 relative group px-4 py-3
                  ${isActive
                    ? 'bg-gradient-to-r from-[#B5FF45]/10 to-transparent text-text-primary'
      ...
      <AnimatePresence>
        {isAIMentorOpen && (
          <AIMentorChat onClose={() => setIsAIMentorOpen(false)} />
        )}
      </AnimatePresence>
    </div>
  );
