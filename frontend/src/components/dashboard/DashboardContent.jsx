...
export function DashboardContent() {
    ...
    /* Continue Preparing */
          ...
          <motion.div variants={itemVariants} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              <ContinueCard icon={Brain} title="Practice Aptitude" subtitle="Quant & Logical" onClick={() => navigate("/aptitude")} />
    ...
    /* Company Preparation */
          ...
          <motion.div variants={itemVariants} className="bg-[#0A0E17] border border-white/5 rounded-3xl p-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
              {["Amazon", "Microsoft", "Google", "Adobe", "Goldman Sachs", "Flipkart", "TCS", "Infosys", "Accenture", "Cognizant"].map((company) => <div key={company} className="bg-white/[0.02] border border-white/5 rounded-2xl p-4 flex flex-col items-center justify-center gap-3 hover:bg-white/[0.05] hover:border-[#B5FF45]/30 hover:shadow-[0_0_15px_rgba(181,255,69,0.05)] hover:-translate-y-1 transition-all duration-300 cursor-pointer group">
  ...
  />
    ...
    animate={{ strokeDasharray: "196 251.2" }}
...
function QuickActionButton({ icon: Icon, label }) {
  return <button className="flex flex-col items-center justify-center gap-2 p-3 rounded-xl bg-white/[0.02] border border-white/5 hover:bg-white/[0.05] hover:border-[#B5FF45]/30 transition-all duration-300 group hover:-translate-y-0.5">
...
function StatCard({ icon: Icon, label, value, trend, highlight }) {
  return <div className={`p-5 rounded-2xl border transition-all duration-300 hover:-translate-y-1 hover:shadow-lg flex flex-col justify-between h-32 group
      ${highlight ? "bg-accent-custom/5 dark:bg-[#B5FF45]/5 border-accent-custom/20 dark:border-[#B5FF45]/20 hover:shadow-accent-custom/10" : "bg-bg-secondary border-border-custom hover:border-border-hover hover:shadow-black/5"}
    `}>
...
function SubjectProgressCard({ icon: Icon, name, progress, chapters, status, color, text }) {
  const statusColors = {
  ...
  return <div className="group bg-white/[0.02] border border-white/5 rounded-2xl p-4 hover:bg-white/[0.04] hover:border-white/10 transition-all duration-300 cursor-pointer">
...
function ContinueCard({ icon: Icon, title, subtitle, onClick }) {
  return <div onClick={onClick} className="group bg-white/[0.02] border border-white/5 rounded-2xl p-4 hover:bg-[#B5FF45]/5 hover:border-[#B5FF45]/20 transition-all duration-300 cursor-pointer flex flex-col items-center text-center hover:-translate-y-1">
...
function TaskItem({ title, duration, priority }) {
  ...
  >
      ...
      <div className="flex-1">
        ...
        <div className="flex items-center gap-3 mt-1.5">
          ...
          {!completed && <span className={`text-[9px] px-1.5 py-0.5 rounded uppercase font-bold tracking-wider border ${pColors[priority]}`}>
              {priority}
            </span>}
        </div>
      </div>
...
function RecommendationItem({ text, alert, positive }) {
  return <div className={`flex items-start gap-3 p-3 rounded-xl border ${alert ? "bg-orange-500/5 dark:bg-orange-500/10 border-orange-500/15 dark:border-orange-500/20 text-orange-800 dark:text-orange-200" : positive ? "bg-emerald-500/5 dark:bg-emerald-500/10 border-emerald-500/15 dark:border-emerald-500/20 text-emerald-800 dark:text-emerald-200" : "bg-item-hover border-border-custom text-text-secondary"}`}>
...
function MockTestCard({ company, name, duration, difficulty, qCount, date }) {
  return <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-4 hover:bg-white/[0.04] transition-colors group">
...
function TimelineItem({ time, title, detail, score, subject, duration, icon: Icon, isAI }) {
