...
export function CompanyPrepContent({ onCompanySelect }) {
  ...
  let filteredCompanies = companiesData.filter((company) => {
    const matchesSearch = company.name.toLowerCase().includes(searchQuery.toLowerCase()) || company.category.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  />
    ...
    className={`px-4 py-2.5 rounded-xl text-[11px] font-semibold whitespace-nowrap transition-all duration-300 ${activeCategory === cat ? "bg-[#B5FF45] text-[#05080D] shadow-[0_0_15px_rgba(181,255,69,0.2)]" : "bg-white/[0.02] text-gray-400 border border-white/5 hover:bg-white/[0.05] hover:text-white"}`}
  >
                {cat}
              </button>)}
          </div>
  ...
  >
              {sortOptions.map((opt) => <option key={opt} value={opt} className="bg-[#05080D]">{opt}</option>)}
            </select>
    ...
    /* Company Grid */
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
          <AnimatePresence>
            {filteredCompanies.map((company, index) => <motion.div
    layout
    key={company.id}
  ...
  >
    ...
    /* Glow Effect */
        ...
        {filteredCompanies.length === 0 && <div className="text-center py-20 bg-[#0A0E17]/60 border border-white/5 rounded-3xl">
    ...
    /* Recommended Companies for You */
      ...
      <motion.div variants={{ hidden: { opacity: 0 }, visible: { opacity: 1 } }} className="pt-8 border-t border-border-custom">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {companiesData.slice(1, 4).map((company) => <div key={`rec-${company.id}`} className="bg-[#0A0E17]/60 border border-[#B5FF45]/20 rounded-2xl p-5 hover:border-[#B5FF45]/50 transition-colors flex items-center justify-between cursor-pointer group" onClick={() => handleCompanyClick(company.id)}>
