...
export function Navbar() {
  ...
  return <>
      <header
    className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-in-out ${scrolled || mobileMenuOpen ? "bg-[#05080D]/85 backdrop-blur-[20px] border-b border-white/5 shadow-[0_4px_30px_rgba(0,0,0,0.7)] py-4" : "bg-transparent py-6"}`}
  >
  ...
  >
    ...
    return <a
      ...
      className={`relative text-[13px] lg:text-[14px] font-medium transition-colors outline-none focus-visible:ring-2 focus-visible:ring-[#B5FF45] rounded-sm py-1 group ${isActive ? "text-[#B5FF45]" : "text-text-secondary hover:text-white"}`}
    >
                  {link.name}
                  <span
      className={`absolute left-0 -bottom-1 h-[2px] bg-[#B5FF45] transition-all duration-300 ease-out ${isActive ? "w-full" : "w-0 group-hover:w-full"}`}
    />
                </a>;
  })}
          </nav>
  ...
  >
    ...
    return <motion.a
      ...
      onClick={(e) => handleNavLinkClick(e, link.sectionId)}
      className={`text-2xl font-medium tracking-tight border-b border-white/5 pb-4 ${isActive ? "text-[#B5FF45]" : "text-text-secondary hover:text-white"}`}
    >
                    {link.name}
                  </motion.a>;
  })}
