...
import { 
  ChevronRight, ArrowLeft, PlayCircle, FileText, CheckCircle2, Lock, 
  Clock, Brain, Search, Bookmark, ChevronDown, Sparkles, Filter, 
  ExternalLink, Calendar, PlusCircle, ArrowRight, Code
} from 'lucide-react';
...
export function SubjectDetailPage() {
  ...
  // Mock state for chapters expansion
  const [expandedChapter, setExpandedChapter] = useState<string | null>(null);
  
  // Fetch subject from database based on id, or fallback to generic
              ...
              {chapters.map((chapter) => (
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4">
                      {!chapter.isLocked && (
                        <ChevronDown className={`w-5 h-5 text-gray-400 transition-transform duration-300 ${expandedChapter === chapter.id ? 'rotate-180 text-white' : ''}`} />
                      )}
                    </div>
                  </button>
          ...
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-3">
              {practiceQuestions.map((q) => (
                <div key={q.id} className="flex items-center justify-between p-4 bg-[#0A0E17]/60 border border-white/5 rounded-2xl hover:border-white/10 transition-colors group">
          ...
          <div className="bg-gradient-to-br from-bg-secondary to-bg-tertiary border border-border-custom rounded-3xl p-6 relative overflow-hidden group">
            ...
            <div className="space-y-2 relative z-10">
              {['Explain Process Synchronization', 'Generate 5 Aptitude Questions'].map((q, i) => (
                <button key={i} className="w-full text-left p-3 rounded-xl bg-item-hover hover:bg-bg-primary text-xs text-text-secondary hover:text-text-primary transition-colors border border-border-custom flex items-center justify-between">
