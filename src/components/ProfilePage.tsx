...
import { 
  User, Mail, Phone, Lock, Bell, Check, LogOut, ArrowLeft,
  ChevronRight, Globe, Eye, EyeOff, Code, Briefcase, Award,
  GraduationCap, Linkedin, Github, MapPin, Sparkles,
  Shield, BookOpen, Terminal, Brain, Cpu, Database, Layers,
  Video, X, Upload, Plus
} from 'lucide-react';
...
export function ProfilePage({ onSignOut, onNavigateToDashboard }: ProfilePageProps) {
  // Navigation & UI States
  const [toast, setToast] = useState<{ message: string; type: 'success' | 'info' | 'error' } | null>(null);
  
  // Custom Profile Editing States
  ...
  // Temporary editing state for the Edit Profile modal
  const [tempProfileData, setTempProfileData] = useState({ ...profileData });
  
  // Tag handling state for visual skills in the edit modal
  const [skillInput, setSkillInput] = useState('');

  // Statistics Count Up Simulation
  ...
  useEffect(() => {
    // Elegant spring/count-up simulation for premium analytics feel
    const duration = 1200;
    const steps = 60;
  ...
  // Subject Preparation Progress Values
  ...
  return (
    <div className="min-h-screen bg-[#05080D] text-white flex flex-col font-sans relative overflow-x-hidden antialiased select-none pb-20">
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <span className={`text-[9px] px-2 py-0.5 rounded-full font-mono ${
                  isConnectedGoogle ? 'bg-[#B5FF45]/10 text-[#B5FF45] border border-[#B5FF45]/20' : 'bg-white/5 text-gray-500'
                }`}>
        ...
        {toast && (
          ...
          >
            <div className={`p-1.5 rounded-lg shrink-0 ${
              toast.type === 'success' ? 'bg-[#B5FF45]/20 text-[#B5FF45]' : 
                ...
                <form onSubmit={handleEditProfileSubmit} className="space-y-5">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      {tempProfileData.skills.split(',').filter(s => s.trim()).map((skill, index) => (
                        <span 
                          key={index}
            ...
            >
              ...
              <div className="space-y-4 pt-2">
                {[
                  ...
                  const active = notifications[item.key as keyof typeof notifications];
                  return (
                    <div key={item.key} className="flex items-center justify-between p-3.5 bg-white/[0.02] border border-white/5 rounded-xl">
                      ...
                      <button 
                        ...
                        className={`px-3 py-1 rounded-full text-[10px] font-mono hover:opacity-90 transition-all ${
                          active 
